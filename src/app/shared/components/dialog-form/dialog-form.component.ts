...
@Component({
      ...
      >
        ...
        <form class="modal-body" (ngSubmit)="onSubmit()">
          <div class="fields-grid">
            <ng-container *ngFor="let field of fields; let i = index; let last = last">
              <div
                class="form-field"
                [class.span-two]="last && fields.length % 2 !== 0"
              >
                <label>
                  {{ field.label }}
                  <span *ngIf="field.required" class="required">*</span>
        ...
        <div class="modal-footer">
          ...
          <button class="save-btn" type="button" (click)="onSubmit()">
            {{ config.submitButtonText || 'Save' }}
          </button>
        </div>
      </div>
  ...
  styles: [`
    ...
    /* Last field spans both columns when total count is odd */
    .form-field.span-two {
      grid-column: 1 / -1;
    }
    ...
    @media (max-width: 600px) {
      ...
      /* span-two is a no-op on single column but keep it harmless */
      .form-field.span-two {
        grid-column: 1;
      }
